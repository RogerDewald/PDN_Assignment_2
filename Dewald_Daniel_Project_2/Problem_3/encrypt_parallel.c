...
/* ------------ Project 2 - Problem 3 - Encryption ------------
    ...
    Just find what in the program can be parallelized and how to encrypt a
   character. Don't forget to log the output time of your modified code!
*/ // ------------------------------------------------------ //
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 6) {
    printf(
        "USE LIKE THIS: encrypt_parallel key input_text.txt output_text.txt time.txt thread_count\n");
    return EXIT_FAILURE;
  }
  ...
  if (!buffer)
    fclose(inputFile), fclose(outputFile),
        // fclose(timeFile),
        free(buffer), fputs("Memory alloc for inputFile1 failed!\n", stderr),
        exit(1);
  ...
  if (1 != fread(buffer, lSize, 1, inputFile))
    fclose(inputFile), fclose(outputFile),
        // fclose(timeFile),
        free(buffer), fputs("Failed reading into the input buffer!\n", stderr),
        exit(2);
  ...
  if (!encrypted_buffer)
    fclose(inputFile), fclose(outputFile),
        // fclose(timeFile),
        free(encrypted_buffer), free(buffer),
...
#pragma omp parallel for num_threads(thread_count)
  for (int i = 0; i < lSize; i++) {
    // encrypted_buffer[i] = ??? ;  // TODO: Encrypt a character from the input
    // buffer.
    encrypted_buffer[i] = ((buffer[i] + key) % 256);
  }
  if (DEBUG)
    printf("Values encypted! \n");
  double end = omp_get_wtime();
  double time = end - start;
  fprintf(timeFile, "%f", time);
